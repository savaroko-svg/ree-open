...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = ImVec2(CalcTextSize(label, NULL, true).x, CalcTextSize(label, NULL, true).y * 1.3333f);

    const float square_sz = GetFrameHeight() * 1.3333f;
    const ImVec2 pos = window->DC.CursorPos;
    ...
    ImVec2 label_pos = ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x * 2.f, check_bb.Min.y + style.FramePadding.y * (1.3333f * 1.3333f));
    if (g.LogEnabled)
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = ImVec2(CalcTextSize(label, NULL, true).x, CalcTextSize(label, NULL, true).y * 2.f);
    ...
    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y * 2.f), label);
